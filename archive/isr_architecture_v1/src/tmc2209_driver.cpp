...
//   2. setRunCurrent() - set IRUN (percent: 0-100)
//   3. setMicrosteps   - set MRES bit field
//   4. enableCoolStep  - turn on CoolStep current reduction
//   5. enable()        - CRITICAL: writes CHOPCONF.TOFF = toff_ (non-zero).
...
void TMC2209Driver::configure(uint16_t current_ma, uint8_t microsteps) {
    ...
    stepper_.enableStealthChop();   // Week 7: force StealthChop before enable().
...
void TMC2209Driver::setupStallGuard(uint8_t sgthrs, uint32_t tcoolthrs,
                                     uint16_t sw_threshold) {
    sw_threshold_ = sw_threshold;
    stall_count_ = 0;
    stall_detected_ = false;
